...
var importObj = (memory, showDebug, inputValue, timestamp, block_height) => ({
    env: {
        ...
        gr_debug: (payload, len) => {
            if (showDebug) {
                console.debug('[GR_DEBUG]', CreateType.create('String', new Uint8Array(memory.buffer.slice(payload, payload + len))).toString());
            }
        },
        gr_error: (error, len) => {
            console.error('[GR_ERROR]', CreateType.create('String', new Uint8Array(memory.buffer.slice(error, error + len))).toString());
        },
        gr_status_code: () => { },
        gr_exit: () => { },
        gr_gas_available: () => { },
